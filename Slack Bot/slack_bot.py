...
def _wiki_call_claude(page_title: str, page_text: str, question: str,
    ...
    prompt = (
        f"다음은 Confluence 검색 결과입니다 (출처: '{page_title}', 1건 이상의 페이지 발췌가 포함될 수 있음):\n\n"
        f"{enrich_ctx}"
        f"{content}{trunc_note}\n\n"
        f"위 검색 결과를 바탕으로 아래 질문에 한국어로 간결하게 답해주세요.\n\n"
        f"[답변 지침]\n"
        f"1. 검색 결과에 여러 페이지가 포함된 경우(`## 제목` 헤더로 구분) — 사용자 질문이 목록/시간범위/필터링이면 해당 페이지들을 통합하여 답변하세요. 페이지 제목·날짜·메타 정보 자체가 답이 될 수 있습니다.\n"
        f"2. 질문에 특정 연도·기간이 명시된 경우, 해당 범위의 데이터만 사용하세요. 다른 연도 데이터와 혼용하지 마세요.\n"
        f"3. 단일 페이지 본문에서 답을 찾는 질문이라면 해당 페이지 내용을 인용하세요. 합계가 명시되어 있으면 그 값을 인용하고, 없으면 해당 범위의 항목을 직접 세어 답하세요. '[검색 관련 섹션]'이 있으면 우선 참고하세요.\n"
        f"4. 검색 결과 전체에 관련 내용이 없으면 '해당 내용을 검색 결과에서 찾을 수 없습니다'라고 답하세요.\n"
...
def _gdi_ask_claude(context_text: str, source_label: str,
    ...
    prompt = (
        f"다음은 GDI 문서 저장소에서 '{source_label}' 관련하여 검색한 결과입니다 "
        f"(`## 제목` 헤더로 구분된 1건 이상의 문서 발췌가 포함될 수 있음).\n"
        f"각 문서에는 [요약]과 [키워드]가 포함될 수 있으며, "
        f"이를 통해 문서의 핵심 맥락을 빠르게 파악할 수 있습니다.\n\n"
        f"{content}{trunc_note}\n\n"
        f"[답변 지침]\n"
        f"- 사용자 질문이 목록/파일명/기간/카테고리이면 검색 결과의 제목·경로·메타 정보 자체가 답이 될 수 있습니다. 통합하여 자연어로 정리하세요.\n"
        f"- 사용자 질문이 단일 문서 내용·요약·분석이면 해당 문서 본문을 바탕으로 답변하세요.\n"
        f"- 질문에만 집중하세요. 질문에서 요청하지 않은 정보(통계, 메타데이터 등)는 포함하지 마세요.\n"
        f"- 검색 결과 전체에 관련 내용이 없으면 '해당 내용을 검색 결과에서 찾을 수 없습니다'라고 답하세요.\n"
        f"{READ_ONLY_INSTRUCTION}"
        f"{ANSWER_FORMAT_INSTRUCTION}\n\n"
        f"질문: {question}"
    )

...
_CLAUDE_SMART_MODEL = os.getenv("CLAUDE_SMART_MODEL", "claude-sonnet-4-5")
import threading
import time
...
# ─────────────────────────────────────────────────────────────────────────────
# Phase 3: 4커맨드 자연어 질의 agentic 라우팅 — 소스별 whitelist/hint (task-193)
# ─────────────────────────────────────────────────────────────────────────────
...
SOURCE_HINTS: dict = {
    "wiki": (
        "\n[소스 제한] 이 질의는 Confluence Wiki 전용."
        " 탐색 순서: ①wiki_search_with_context(캐시 우선)"
        " ②0건이면 wiki_search_content_live(실시간 본문검색) 필수 시도."
        " 캐시 0건만으로 '없다' 결론 금지."
    ),
    "gdi": (
        "\n[소스 제한] GDI 문서 전용."
        " ①메타 축(gdi_get_build_index: game/file_kind/날짜)"
        " ②관련 자료군(get_related_nodes) ③gdi_unified_search(본문) 순."
        " 1건 찾고 종료 금지."
    ),
    "jira": (
        "\n[소스 제한] Jira 전용."
        " 게임명은 text ~ 횡단 검색(프로젝트 키 추측 금지)."
        " 기간 질의는 created/updated 필드 활용."
        " 0건이면 동의어/기간 완화 재시도."
    ),
    "biskit": (
        "\n[소스 제한] BISKIT 지표 전용."
        " 카탈로그로 메뉴/데이터셋 파악 → search_datasets(동의어 모아 한번에)"
        " → get_dataset_parameters로 스키마 확인 → execute_query."
        " 감정/긍부정 질문은 '감정 분포' 단일 데이터셋(비교분석 리포트 아님) 우선."
        " 0값이면 lang=KO→EN, perdCd D↔W 재시도."
    ),
}
...
_STR_LEN_CAP = 2000  # 모든 문자열 인자 최대 길이
_SPACE_KEY_RE = _re.compile(r'^[A-Za-z0-9_-]{1,32}$')


def _dict_depth(d, current: int = 0) -> int:
    """dict 중첩 깊이를 반환한다. 비-dict이면 current를 그대로 반환."""
...
def _validate_tool_args(name: str, args: dict) -> str:
    ...
    # ── 공통: 모든 str 인자 길이 cap (2000자 초과 차단) ──────────────────────
    for _val in args.values():
        if isinstance(_val, str) and len(_val) > _STR_LEN_CAP:
            return "[보안] 인자 값이 너무 깁니다 (최대 2000자)."
    ...
    if name == "jira_search_issues":
        ...
        if any(p in jql for p in ("or1=1", ";", "/*", "*/")):
            return "[보안] 허용되지 않은 쿼리 패턴입니다."
    # task-193: get_related_nodes rel_type enum 화이트리스트 + source_id traversal 차단
    if name == "get_related_nodes":
        ...
        if ".." in sid or "\n" in sid or "\r" in sid:
            return "[보안] 잘못된 source_id 형식입니다."
    # wiki_search_with_context space_key — 화이트리스트 (빈값 허용)
    if name == "wiki_search_with_context":
        sk = str(args.get("space_key", ""))
        if sk and not _SPACE_KEY_RE.match(sk):
            return "[보안] space_key는 영문/숫자/언더스코어/하이픈 1~32자만 허용됩니다."
    # biskit_execute_query parameters — dict 깊이/키 개수 cap
    if name == "biskit_execute_query":
        params = args.get("parameters")
        if isinstance(params, dict):
            if len(params) > 50:
                return "[보안] parameters 키 개수가 너무 많습니다 (최대 50개)."
            if _dict_depth(params) > 3:
                return "[보안] parameters 중첩 깊이가 너무 깊습니다 (최대 3단계)."
    return ""
...
def run_agentic(
    ...
    token_spent = 0
    _last_hb = 0.0
    _consec_all_fail = 0   # E-3: 연속 전(全)실패 iteration 카운터
    ...
    for i in range(MAX_ITER):
        ...
        if resp.stop_reason == "tool_use":
            ...
            tool_results = []
            _iter_fail_count = 0   # E-3: 이번 iteration 실패 도구 수
            _iter_tool_count = 0   # E-3: 이번 iteration 전체 도구 수
            for block in resp.content:
                if getattr(block, "type", None) == "tool_use":
                    result_str = registry.dispatch(block.name, block.input)
                    _iter_tool_count += 1
                    if result_str.startswith("[도구"):
                        _iter_fail_count += 1
                    tool_results.append({
                        "type": "tool_result",
            ...
            # E-3: 전(全)실패 연속 감지
            if _iter_tool_count > 0 and _iter_fail_count == _iter_tool_count:
                _consec_all_fail += 1
                logger.warning(
                    f"[agentic] iter={i} 전체 도구 실패 "
                    f"({_iter_fail_count}/{_iter_tool_count}), "
                    f"연속={_consec_all_fail}"
                )
                if _consec_all_fail >= 2:
                    logger.error(
                        f"[agentic] 연속 {_consec_all_fail}회 전체 도구 실패 — 조기 종료"
                    )
                    return "외부 데이터 서비스 연결이 불안정합니다. 잠시 후 다시 시도해주세요."
            else:
                _consec_all_fail = 0   # 하나라도 성공하면 리셋
